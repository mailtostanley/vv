...
from ctaBase import *
from mctaSetting import MSTRATEGY_CLASS
...
########################################################################
class MultiCtaEngine(object):
    """CTA策略引擎"""
    settingFileName = 'MCTA_setting.json'
    ...
    def sendStopOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        # 保存stopOrder对象到字典中，只是放到本地，并未发送出去。
    ...
    def processStopOrder(self, tick):
        """收到行情后处理本地停止单（检查是否要立即发出，如果有需要发出的停止单，直接发出）"""
    ...
    def loadStrategy(self, setting):
        ...
        # 获取策略类
        strategyClass = MSTRATEGY_CLASS.get(className, None)
        ...
        if name in self.strategyDict:
            ...
            # 保存Tick映射关系
            for s in strategy.vtSymbolList:
                if s in self.tickStrategyDict:
                    l = self.tickStrategyDict[s]
                else:
                    l = []
                    self.tickStrategyDict[s] = l
                l.append(strategy)
                ...
                if contract:
                    ...
                    self.writeCtaLog(u'%s的交易合约%s无法找到' %(name, s))
    ...
    def savePosition(self):
        ...
        for strategy in self.strategyDict.values():
            flt = {'name': strategy.name,
                   'vtSymbol': strategy.vtSymbolList}
