...
class DrEngine(object):
        ...
        with open(self.settingFileName) as f:
            ...
            if 'bar' in drSetting:
                ...
                for setting in l:
                    ...
                    dayBar = DrBarData()
                    self.dayBarDict[vtSymbol] = dayBar
    ...
    def procecssTickEvent(self, event):
        ...
        # 更新日线数据
        if vtSymbol in self.dayBarDict:
            bar = self.dayBarDict[vtSymbol]
            
            # 如果第一个TICK或者新的一分钟
            if not bar.datetime or bar.datetime.day != drTick.datetime.day:
                if bar.vtSymbol:
                    newBar = copy.copy(bar)
                    self.insertData(DAILY_DB_NAME, vtSymbol, newBar)
                    
                    if vtSymbol in self.activeSymbolDict:
                        activeSymbol = self.activeSymbolDict[vtSymbol]
                        self.insertData(DAILY_DB_NAME, activeSymbol, newBar)                    
    ...
    #----------------------------------------------------------------------
    def listActiveContracts(self):
        """打印出所有活跃合约"""
        activecontractSymbol = []
        #zs
        for c in CONTRACT_CODE_ZS:
            for m in CURRENT_ACTIVE_MONTH_159:
                activecontractSymbol.append({c : c + CURRENT_ACTIVE_YEAR_ZS
                                             + m})
        #zs
        for c in CONTRACT_CODE_DS:
            for m in CURRENT_ACTIVE_MONTH_159:            
                activecontractSymbol.append({c : c + CURRENT_ACTIVE_YEAR_DS
                                             + m})

        #SH159
        for c in CONTRACT_CODE_SH159:
            for m in CURRENT_ACTIVE_MONTH_159:            
                activecontractSymbol.append({c : c + CURRENT_ACTIVE_YEAR_SH159
                                             + m})

        #SH 612
        for c in CONTRACT_CODE_SH612:
            for m in CURRENT_ACTIVE_MONTH_612:                        
                activecontractSymbol.append({c : c + CURRENT_ACTIVE_YEAR_SH612
                                             + m})

        #SH 612
        for c in CONTRACT_CODE_SHALL:
            for m in CURRENT_ACTIVE_MONTH_ALL:                                    
                activecontractSymbol.append({c : c + CURRENT_ACTIVE_YEAR_SHALL
                                             + m})

        print "total contracts: " + str(len(activecontractSymbol))
