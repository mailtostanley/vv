...
class BasicMonitor(QtGui.QTableWidget):
    ...
    #----------------------------------------------------------------------
    def updateStrategyData(self, data):
        """将数据更新到表格中"""
        # 如果允许了排序功能，则插入数据前必须关闭，否则插入新的数据会变乱
        if self.sorting:
            self.setSortingEnabled(False)
        ...
        if self.dataKey:
            ...
            if key not in self.dataDict:
                ...
                d = {}
                for n, header in enumerate(self.headerList):
                    content = safeUnicode(data[header])
                    cellType = self.headerDict[header]['cellType']
                    cell = cellType(content, self.mainEngine)
...
########################################################################
class StrategyMonitor(BasicMonitor):
    """strategy 组件"""

    #----------------------------------------------------------------------
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(StrategyMonitor, self).__init__(mainEngine, eventEngine, parent)

        # 设置表头有序字典
        d = OrderedDict()
        d['time'] = {'chinese':u'time', 'cellType':BasicCell}              
        d['symbol'] = {'chinese':u'合约代码', 'cellType':BasicCell}      
        d['lastPriceList'] = {'chinese':u'最新', 'cellType':BasicCell}              
        d['openVolume'] = {'chinese':u'开仓手数', 'cellType':BasicCell}
        d['buyPriceList'] = {'chinese':u'买开价格', 'cellType':BasicCell}
        d['shortPriceList'] = {'chinese':u'卖开价格', 'cellType':BasicCell}
        d['stopPriceList'] = {'chinese':u'止损价格', 'cellType':BasicCell}
        d['turtleState'] = {'chinese':u'状态', 'cellType':BasicCell}
        d['tDirection'] = {'chinese':u'方向', 'cellType':BasicCell}                
        d['tFirstPrice'] = {'chinese':u'1Price', 'cellType':BasicCell}        
        d['tFirstPos'] = {'chinese':u'1Pos', 'cellType':BasicCell}         
        d['tFirstDate'] = {'chinese':u'1Date', 'cellType':BasicCell}        
        d['tSecondPrice'] = {'chinese':u'2Price', 'cellType':BasicCell}        
        d['tSecondPos'] = {'chinese':u'2Pos', 'cellType':BasicCell}         
        d['tSecondDate'] = {'chinese':u'2Date', 'cellType':BasicCell}        
        d['tThirdPrice'] = {'chinese':u'3Price', 'cellType':BasicCell}         
        d['tThridPos'] = {'chinese':u'3Pos', 'cellType':BasicCell}        
        d['tThirdDate'] = {'chinese':u'3Date', 'cellType':BasicCell}         
        d['tFourthPrice'] = {'chinese':u'4Price', 'cellType':BasicCell}        
        d['tFourthPos'] = {'chinese':u'4Pos', 'cellType':BasicCell}         
        d['tFourthDate'] = {'chinese':u'4Date', 'cellType':BasicCell}                
        d['atrValue'] = {'chinese':u'ATR', 'cellType':BasicCell}
        d['tenDayHigh'] = {'chinese':u'十日最高', 'cellType':BasicCell}
        d['tenDayLow'] = {'chinese':u'十日最低', 'cellType':BasicCell}
        d['twentyDayHigh'] = {'chinese':u'二十日高', 'cellType':BasicCell}
        d['twentyDayLow'] = {'chinese':u'二十日低', 'cellType':BasicCell}
        self.setHeaderDict(d)
        ...
        # 设置监控事件类型
        #self.setEventType(EVENT_CTA_MONITOR)
